...
exports.applyForRooom = async (req, res) => {
  ...
  const message = `${firstName}  is requesting for your Car with ${req.params.id} to be approved `;
  ...
  return res
    .status(200)
    .json({ info: "Your request has been sent for Approval" });
};

exports.availableForBooking = async (req, res) => {
  const bookingDetails = await bookingDetailsModel.findOne({
...
exports.checkOut = async (req, res) => {
  ...
  const checkOutRoom = await rentDetailsModel.findByIdAndUpdate(
    req.params.id,
    { $set: { booked: false } },
    { new: true }
  );
