...
export class ListRentDetailsComponent implements OnInit {
  ...
  info(row: any) {
    const viewData = { ...row }; // Clone the row object
    delete viewData.image; // Remove image data from the cloned object
    delete viewData.legalDocuments; // Remove legal documents data

    localStorage.setItem('viewRoom', JSON.stringify(viewData));
